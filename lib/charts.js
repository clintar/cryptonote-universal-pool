...
function startDataCollectors() {
	...
	var donationsettings = config.charts.user.donationHashrate;
    if(donationsettings.enabled) {
        setInterval(function() {
            collectUsersDonationHashrate('donationHashrate', donationsettings);
        }, donationsettings.updateInterval * 1000);
    }
}
...
function getUserDonationHashrateChartData(address, callback) {
    getChartDataFromRedis('donationHashrate:' + address, callback);
}
...
function getUserChartsData(address, paymentsData, callback) {
    ...
    var chartsFuncs = {
        ...
        donationHashrate: function(callback) {
            getUserDonationHashrateChartData(address, function(data) {
                callback(null, data);
            });
        },
...
function getUsersHashrates(callback) {
    apiInterfaces.pool('/miners_hashrate', function(error, data) {
        log('warn', logSystem, 'userhashrate data - ', [data]);
        callback(data.minersHashrate);
    });
}

function getUsersDonationHashrates(callback) {
    apiInterfaces.pool('/miners_donationHashrate', function(error, data) {
        callback(data.minersHashrate);
    });
}
...
function collectUsersDonationHashrate(chartName, settings) {
    var redisBaseKey = getStatsRedisKey(chartName) + ':';
    redisClient.keys(redisBaseKey + '*', function(keys) {
		var donationHashrates = {};
		for(var i in keys) {
            donationHashrates[keys[i].substr(keys[i].length)] = 0;
        }
		getUsersDonationHashrates(function(newHashrates) {
            for(var address in newHashrates) {
                donationHashrates[address] = newHashrates[address];
            }
            storeCollectedValues(chartName, donationHashrates, settings);
        });
    });
}
