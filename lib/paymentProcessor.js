            ...
            apiInterfaces.rpcWallet('getbalance', '', function(error, result){
                ...
                async.waterfall([
                    ...
                    function(blocks, callback){
                        ...
                        {
                            log('info', logSystem, 'Potential current pool reserves (assuming payments processed and blocks will mature) after payout to miners: %d ', [(result.balance  + ((totalPendingPoolReward * feePercent) ) - (totalPendingPoolReward * ((100 - config.blockUnlocker.poolFee) / 100) )) / config.coinUnits]);
                        }
                    }
                ], function(error, result){
