...
function checkLogin(login, sendReply, callback)
{
	...
	if(login.indexOf('@') === 0)
	{
		...
		log('warn', logSystem, "checking %s for alias", [login]);
		apiInterfaces.rpcDaemon('get_alias_details', {alias: login} , function (error, result)
		{
			...
			log('warn', logSystem, "alias result %j", [result]);
			log('warn', logSystem, "result.alias_details.address %s ", [result.alias_details.address]);
			if ( result.status !== "OK" )
			{
				log('warn', logSystem, "login is (pre1) %s", [login]);
			...
			log('warn', logSystem, "login is (pre2) %s", [login]);
...
function processBlockTemplate(template){
    ...
    getFullScratchpad2(function (error) {
        if (error) {
            log('error', logSystem, 'Failed to get scratchpad: ' + error.message);
        }
    for (var minerId in connectedMiners){
        //XMR FIXME:
        //var miner = connectedMiners[minerId];
...
function reloadAliasesQue()
{
    ...
    {
        log('debug', logSystem, 'Loaded aliases que, ' + aliases_config.aliases_que.length + ' items');
    }
...
function Miner(id, login, pass, ip, startingDiff, noRetarget, pushMessage, donationAddress, donationPercentage){
    this.id = id;
    this.login = login;
    this.pass = pass;
    this.ip = ip;
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ['hincrby', config.coin + ':shares:roundCurrent', miner.login, job.difficulty * ((100 - miner.donationPercentage) / 100)],
	...
	if(miner.hasDonation === true || true)
	{
		var redisCommands = [
			['hincrby', config.coin + ':shares:roundCurrent', miner.donationAddress, job.difficulty * ((miner.donationPercentage) / 100)],
			['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty * ((miner.donationPercentage) / 100), miner.donationAddress, dateNow].join(':')],
			['hincrby', config.coin + ':workers:' + miner.donationAddress, 'hashes', job.difficulty * ((miner.donationPercentage) / 100)],
			['hset', config.coin + ':workers:' + miner.donationAddress, 'lastShare', dateNowSeconds]
		];
		redisClient.multi(redisCommands).exec(function(err, replies){
			if (err){
				log('error', logSystem, 'Failed to insert share data into redis %j \n %j', [err, redisCommands]);
		...
		log('info', logSystem, 'Accepted %s share at difficulty %d/%d from %s@%s', [shareType, job.difficulty * ((miner.donationPercentage) / 100), shareDiff, miner.donationAddress, miner.ip]);
	}
}
        ...
        case 'login':
            ...
            if(config.poolServer.fixedDiff.enabled) {
                ...
                if(loginEndCharPostion !== -1) {
                    noRetarget = true;
                    difficulty = login.substr(loginEndCharPostion + 1);
			...
			log('warn', logSystem, "login is %s", [login]);
            if(config.poolServer.userDonations.enabled) {
                var userDonationsAddressCharPos = login.indexOf(config.poolServer.userDonations.addressSeparator);
				var userDonationPercentageCharPos = login.indexOf(config.poolServer.userDonations.percentSeparator);
                if(userDonationsAddressCharPos !== -1 && userDonationPercentageCharPos !== -1) {
					
					if(userDonationsAddressCharPos < userDonationPercentageCharPos)
					{
						loginEndCharPostion = userDonationsAddressCharPos;
						donationAddress = login.substr(userDonationsAddressCharPos + 1, userDonationPercentageCharPos - userDonationsAddressCharPos -  1);
						donationPercentage = login.substr(userDonationPercentageCharPos + 1);
					}
					else
					{
						loginEndCharPostion = userDonationPercentageCharPos;
						donationAddress = login.substr(userDonationsAddressCharPos + 1);
						donationPercentage = login.substr(userDonationPercentageCharPos + 1, userDonationsAddressCharPos - userDonationPercentageCharPos - 1);
					}
                    if(donationPercentage < config.poolServer.userDonations.minDonation) {
						log('info', logSystem, 'Miner set too low donation %s, min: %s%%',  [donationPercentage, config.poolServer.userDonations.minDonation]);
                        sendReply('donation set below minimum');
						return;
                    }
					if(donationPercentage > config.poolServer.userDonations.maxDonation) {
						log('info', logSystem, 'Miner set too high donation %s, max: %s%%',  [donationPercentage, config.poolServer.userDonations.maxDonation]);
                        sendReply('donation set above maximum');
						return;
                    }
                    
					if (addressBase58Prefix !== cnUtil.address_decode(new Buffer(donationAddress))){
						sendReply('invalid donation address');
						return;
					}
					log('info', logSystem, 'Miner set donation for %s, to %s%%',  [donationAddress, donationPercentage]);
                }
            }
			log('warn', logSystem, "login was %s", [login]);
			if(loginEndCharPostion !== -1) {
				login = login.substr(0, loginEndCharPostion);
			}
			log('warn', logSystem, "login is  %s", [login]);
			...
			checkLogin(login, sendReply, function(error, result)
			{
				if (error !== false)
				{
					log('warn', logSystem, "login error: %s", [result]);
					sendReply('invalid address used for login');
					return;
				}
				login = result;
				log('warn', logSystem, "login is (pre3) %s", [login]);
				...
				log('info', logSystem, 'Miner set donation for %s, to %s%%',  [donationAddress, donationPercentage]);
				miner = new Miner(minerId, login, params.pass, ip, difficulty, noRetarget, pushMessage, donationAddress, donationPercentage);
				if(params.hi
