...
function getFullScratchpad2(callback)
{
    ...
    apiInterfaces.binRpcDaemon('/getfullscratchpad2', {}, function(er, buff)
    {
        ...
        var gotScratchpad = +new Date();
        log('debug', logSystem, 'Time to request scratchpad: %d', [gotScratchpad - requestStart]);
        ...
        log('debug', logSystem, 'Time to parse scratchpad: %d', [requestEnd - gotScratchpad]);
        callback(null);
...
setInterval(function(){
    ...
    for (var minerId in connectedMiners){
        ...
        if (now - miner.lastBeat > timeout){
            log('warn', logSystem, 'Miner timed out and disconnected %s@%s', [miner.account, miner.ip]);
...
function getAddms(hi, callback) {
    apiInterfaces.rpcDaemon('getjob', {id: '', hi: hi}, function(error, result) {
        if (error){
            log('error', logSystem, 'Error polling getjob %j', [error]);
            callback(false);
            return;
        }
...
function processBlockTemplate(template){
    ...
    getFullScratchpad2(function (error) {
        ...
        var start = +new Date();    
        async.each(connectedMiners, function(miner, cb) {
            //XMR FIXME:
        ...
        var end = +new Date();
        log('debug', logSystem, 'Time to send to miners: %d', [end - start]);
...
function Miner(id, login, pass, ip, startingDiff, noRetarget, pushMessage, donationAddresses, donationPercentage){
    ...
    if (this.pass.match(/^[a-f0-9]{64}$/i) != null) {
        this.account = [this.login, this.pass].join(':');
    }
    else {
...
Miner.prototype = {
    retarget: function(now){
        ...
        this.numRetargets = this.numRetargets + 1;
        if (options.maxIterations)
        {
            if (this.numRetargets > options.maxIterations)
            {
                log('info', logSystem, 'Retargetting has reached max iterations for %s, difficulty set to %d', [this.account, newDiff]);
                this.noRetarget = true;
            }
            else
            {
                log('info', logSystem, 'Retargetting... maxIiterations:%d/%d not reached for %s', [this.numRetargets, options.maxIterations, this.account]);
            }
        }
    ...
    setNewDiff: function(newDiff){
        ...
        log('info', logSystem, 'Retargetting difficulty %d to %d for %s', [this.difficulty, newDiff, this.account]);
    ...
    checkBan: function(validShare){
        ...
        if (stats.validShares + stats.invalidShares >= config.poolServer.banning.checkThreshold){
            if (stats.invalidShares / stats.validShares >= config.poolServer.banning.invalidPercent / 100){
                log('warn', logSystem, 'Banned %s@%s', [this.account, this.ip]);
    ...
    fetchAddms: function(callback) {
        ...
        var miner = this;
        log('info', logSystem, 'miner height requested: ',[this.hi.height]);
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ['hincrby', config.coin + ':shares:roundCurrent', miner.account, parseInt(job.score * ( 100 - parseFloat(miner.donationPercentage) ) / 100)],
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, miner.account, dateNow].join(':')],
        ['hincrby', config.coin + ':workers:' + miner.account, 'hashes', parseInt(job.difficulty * ( 100 - parseFloat(miner.donationPercentage) ) / 100)],
        ['hset', config.coin + ':workers:' + miner.account, 'lastShare', dateNowSeconds]
    ];
	if(miner.hasDonation === true)
	{
		for (var i=0; i < miner.donations.length;i++)
		{	
			redisCommands.push(['hincrby', config.coin + ':shares:roundCurrent', miner.donations[i][0], parseInt(job.score * (miner.donations[i][1] / 100))]);
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d from %s@%s', [shareType, job.difficulty * ((100 - miner.donationPercentage) / 100), shareDiff, miner.account, miner.ip]);
	for (var i=0; i < miner.donations.length;i++)
		{
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (hash.toString('hex') !== resultHash) {
        log('warn', logSystem, 'Bad hash from miner ' +  miner.account + '@' + miner.ip +
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                log('error', logSystem, 'Error submitting block at height %d from %s@%s, share type: "%s" - %j', [job.height, miner.account, miner.ip, shareType, error]);
                ...
                log('info', logSystem,
                    ...
                    [blockFastHash.substr(0, 6), job.height, miner.account, miner.ip, result]
                );
        ...
        log('warn', logSystem, 'Rejected low difficulty share of %s from %s@%s', [hashDiff.toString(), miner.account, miner.ip]);
        return false;
    }
    else{
        recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
   ...
   switch(method){
        case 'login':
			...
			checkDonations(donationAddresses, sendReply, parsedAddresses,function(error, donationOutput)
			{
                ...
                if(params.hi && (currentBlockTemplate.height - params.hi.height) > 2160)
                {
                    sendReply('invalid miner height, re-download scratchpad');
                }
				checkLogin(login, sendReply, function(error, result)
				{
					...
					if(params.hi
                        ...
                        {
                            log('warn', logSystem, 'method ' + method + ', miner have height=' + miner.hi.height + ', currentBlockTemplate.height=' + currentBlockTemplate.height + ', refreshing job');
                        }
					}
        ...
        case 'submit':
            ...
            if (!noncePattern.test(params.nonce)) {
                 var minerText = miner ? (' ' + miner.account + '@' + miner.ip) : '';
           ...
           if (job.submissions.indexOf(params.nonce) !== -1){
            ...
            if (shareTrustEnabled){
                if (shareAccepted){
                    ...
                    log('warn', logSystem, 'Share trust broken by %s@%s', [miner.account, miner.ip]);
        ...
        case 'getfullscratchpad':
            ...
            // this has gone beyond what node can handle, so giving up on full scratchpad now
