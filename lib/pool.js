...
Miner.prototype = {
    retarget: function(now){
        ...
        if (options.maxIterations)
        {
            ...
            {
                log('info', logSystem, 'Retargetting has reached max iterations for %s, difficulty set to %d', [this.login, newDiff]);
                this.noRetarget = true;
            }
            else
            {
                log('info', logSystem, 'Retargetting... maxIiterations:%d/%d not reached for %s', [this.numRetargets, options.maxIterations, this.login]);
            }
        }
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ['hincrby', config.coin + ':shares:roundCurrent', miner.login, parseInt(job.score * ( 100 - parseFloat(miner.donationPercentage) ) / 100)],
